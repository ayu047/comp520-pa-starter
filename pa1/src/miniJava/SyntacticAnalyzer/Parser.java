...
public class Parser {
	...
	private MemberDecl parseFDorMD() throws SyntaxError {
		...
		if (_currentToken.getTokenType() == TokenType.Void) {
			...
			accept(TokenType.RCurly);
			return new MethodDecl(new FieldDecl(isPrivate, isStatic, td, name, _currentToken.getTokenPosition()), pdl,
					sl, _currentToken.getTokenPosition());
		} else {
			td = parseType();
			name = _currentToken.getTokenText();
			accept(TokenType.Identifier);
			if (_currentToken.getTokenType() == TokenType.LParen) {
				...
				accept(TokenType.RCurly);
				return new MethodDecl(new FieldDecl(isPrivate, isStatic, td, name, _currentToken.getTokenPosition()),
						pdl, sl, _currentToken.getTokenPosition());
			} else {
	...
	private TypeDenoter parseType() throws SyntaxError {
		if (_currentToken.getTokenType() == TokenType.INT) {
			...
			if (_currentToken.getTokenType() == TokenType.LSquare) {
				...
				return new ArrayType(new BaseType(TypeKind.INT, _currentToken.getTokenPosition()),
			...
			if (_currentToken.getTokenType() == TokenType.LSquare) {
				accept(TokenType.LSquare);
				accept(TokenType.RSquare);
				return new ArrayType(new ClassType(new Identifier(t), _currentToken.getTokenPosition()),
						_currentToken.getTokenPosition());
			}
		...
		if (_currentToken.getTokenType() == TokenType.LCurly) {
			...
			if (_currentToken.getTokenType() == TokenType.LSquare) {
				...
				if (_currentToken.getTokenType() == TokenType.RSquare) {
					...
					Expression e = parseExpression();
					accept(TokenType.Semicolon);
					TypeDenoter td = new ArrayType(new ClassType(new Identifier(t), _currentToken.getTokenPosition()),
							_currentToken.getTokenPosition());
					VarDecl vd = new VarDecl(td, name, _currentToken.getTokenPosition());
					return new VarDeclStmt(vd, e, _currentToken.getTokenPosition());
				} else {
	...
	private Expression parseOrExpression() throws SyntaxError {
		Expression exp = parseAndExpression();
		while (_currentToken.getTokenText().equals("||")) {
			Token t = _currentToken;
			accept(TokenType.Operator);
			Operator o = new Operator(t);
			Expression e = parseAndExpression();
			exp = new BinaryExpr(o, exp, e, _currentToken.getTokenPosition());
		}
		return exp;
	}
	
	private Expression parseAndExpression() throws SyntaxError {
		Expression exp = parseEqlExpression();
		while (_currentToken.getTokenText().equals("&&")) {
			Token t = _currentToken;
			accept(TokenType.Operator);
			Operator o = new Operator(t);
			Expression e = parseEqlExpression();
			exp = new BinaryExpr(o, exp, e, _currentToken.getTokenPosition());
		}
		return exp;
	}
	
	private Expression parseEqlExpression() throws SyntaxError {
		Expression exp = parseComExpression();
		while (_currentToken.getTokenText().equals("==") || _currentToken.getTokenText().equals("!=")) {
			Token t = _currentToken;
			accept(TokenType.Operator);
			Operator o = new Operator(t);
			Expression e = parseComExpression();
			exp = new BinaryExpr(o, exp, e, _currentToken.getTokenPosition());
		}
		return exp;
	}
	
	private Expression parseComExpression() throws SyntaxError {
	...
	private Expression parseAddExpression() throws SyntaxError {
		Expression exp = parseMulExpression();
		while (_currentToken.getTokenText().equals("+") || _currentToken.getTokenText().equals("-")) {
			Token t = _currentToken;
			accept(TokenType.Operator);
			Operator o = new Operator(t);
			Expression e = parseMulExpression();
			exp = new BinaryExpr(o, exp, e, _currentToken.getTokenPosition());
		}
		return exp;
	}
	
	private Expression parseMulExpression() throws SyntaxError {
	...
	private Expression parseUnaryExpression() throws SyntaxError {
		if (_currentToken.getTokenText().equals("-") || _currentToken.getTokenText().equals("!")) {
			...
			Expression e = parseUnaryExpression();
			Operator o = new Operator(t);
			return new UnaryExpr(o, e, _currentToken.getTokenPosition());
		} else {
			return parseOtherExpression();
		}
	}

	private Expression parseOtherExpression() throws SyntaxError {
		Expression exp = null;
		if (_currentToken.getTokenType() == TokenType.This || _currentToken.getTokenType() == TokenType.Identifier) {
			Reference r = parseReference();
			...
			Terminal t = new NullLiteral(token);
			exp = new LiteralExpr(t, _currentToken.getTokenPosition());
